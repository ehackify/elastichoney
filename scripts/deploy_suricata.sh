...
mkdir -p /opt/suricata/etc/suricata/rules /opt/hnp/rules/
...
# Register the sensor with hnp server.
...
# hnp.rules will be used as local.rules.
rm -f /opt/suricata/etc/suricata/rules/local.rules
ln -s /opt/hnp/rules/hnp-suricata.rules /opt/suricata/etc/suricata/rules/local.rules
...
mkdir -p /opt/hnp/rules
rm -f /opt/hnp/rules/hnp.rules.tmp

echo "[`date`] Updating suricata signatures ..."
wget $server_url/static/hnp.rules -O /opt/hnp/rules/hnp-suricata.rules.tmp && \
	mv /opt/hnp/rules/hnp-suricata.rules.tmp /opt/hnp/rules/hnp-suricata.rules && \
